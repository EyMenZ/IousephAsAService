...
public class DeezerParser implements IParser {
	...
	public List<Playlist> playlistsParse(JSONObject json) {
		...
		try {
			while (((JSONObject) json.getJSONArray("data").get(i)) != null) {
				playlists.add(playlistParse(((JSONObject) json.getJSONArray("data").get(i))));
				i++;
			}
		} catch (JSONException e) {
			return playlists;
	...
	public List<Track> playlistIdParse(JSONObject json) {
		/*
		 * List<Track> tracks = new ArrayList<>(); int i = 0;
		 * System.out.println(((JSONObject)
		 * (json.getJSONArray("tracks")).get(0))); try { while (((JSONObject)
		 * (json.getJSONArray("tracks")).get(i)) != null) {
		 * tracks.add(trackParse(((JSONObject) json.getJSONArray("tracks")
		 * .get(i)))); i++; } } catch (JSONException e) { return tracks; }
		 */
